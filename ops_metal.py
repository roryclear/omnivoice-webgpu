...
class MetalAllocator(LRUAllocator[MetalDevice]):
  def __init__(self, dev, **kwargs):
    ...
    self.dev = dev
    self.num_bufs = -1 # 1 added
  def _alloc(self, size:int, options) -> MetalBuffer:
    if options.external_ptr:
      print("PTR")
      exit()
      return MetalBuffer(metal.MTLBuffer(options.external_ptr), size)
    ...
    self.num_bufs+=1
    self.dev.q.append({"buff_alloc":{"num":self.num_bufs, "size":size}})
    return MetalBuffer(ret, size, num=self.num_bufs)
  @suppress_finalizing
